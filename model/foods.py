...
class Food(db.Model):
    ...
    # Define the food schema with "vars" from object
    id = db.Column(db.Integer, primary_key=True)
    _name = db.Column(db.String(255), unique=False, nullable=False)
    _uid = db.Column(db.String(255), unique=False, nullable=False)
    _tokens = db.Column(db.Float)    

    # constructor of a food object, initializes the instance variables within object (self)
    ...
    def create(self):
        try:
            # creates a food object from food(db.Model) class, passes initializers
        ...
        for key in dictionary:
            if key == "name":
                if self.name == dictionary[key]:
                    namefound = True
            if key == "uid":
                if self.uid == dictionary[key]:
                    uidfound = True
            if key == "tokens":
                token = dictionary[key]
        if namefound == True and uidfound == True:
            self.tokens = token
            db.session.commit()
        return self
...
def initFoods():
    ...
    with app.app_context():
        ...
        foods = [
            Food(name='Josh Williams', uid='joshW', tokens=1.0),
            Food(name='John Mortensen', uid='johnM', tokens=2.0)
        ]
